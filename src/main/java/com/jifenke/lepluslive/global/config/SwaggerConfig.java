...
public class SwaggerConfig {
  ...
  public Docket union_payApi() {
    return new Docket(DocumentationType.SWAGGER_2)
        ...
        .pathMapping("/")// base，最终调用接口后会和paths拼接在一起
        .select()
        .paths(or(regex("/lepay/union_pay/.*")))//过滤的接口
        .build()
        .apiInfo(unoin_payInfo());
  }
...
//小标题
  ...
  private ApiInfo unoin_payInfo() {
    ApiInfo apiInfo = new ApiInfo("银联主调接口相关操作",//大标题
                                  "EHR Platform's REST API, all the applications could access the Object model data via JSON.",
//小标题
                                  "0.1",//版本
                                  "NO terms of service",
